...
I want to create an application that can measure the performance of drummers practicing on a drum pad. The application should track sticks on a video of a drummer playing on a drum pad. The tracked sticks can be used to record the playing and calculate some metrics, for example the stick height on left and right hand, the straightness of the hits or the angle of the hits. These metrics can help the player to improve their drumming.
For this project I chose the project category “bring your own data”.
## Background Research
There are several approaches to tracking drumsticks with deep learning technologies. The majority of the methods are built to enable air drumming, which means moving sticks in the air without an actual drumset. Hong et al. [1] propose a framework that can detect drumsticks with markers on the tip using YOLOv8. Yadid et al. [2] provide a markerless approach with the YOLOv5 model. Both approaches give valuable insights into their dataset creation and network training but have not published their datasets. Since both focus on air drumming, they did not train their networks on frames of people drumming on an actual drum pad.
...
## Planned Approach
A major part of the project will be dataset creation. I plan to use a smartphone camera and a tripod to record videos from different angles. The dataset will then be split into frames and annotated with Roboflow [5] or CVAT [6]. Rotation and brightness changes will be applied to reduce the number of necessary samples and introduce robustness. For training, I will use a pre-trained network. In a final application, a platform to upload a video will be provided. The application can then calculate some easy metrics like stick height.
## Schedule
### Planned Schedule
...
data/
    ...
    labels/
        ...
        val/
```
The actual training images belong to the images/train folder, whereas for each image an annotation text file belongs into the labels/train folder. Image and label files must have the same file name.
These files will be used by the YOLO framework during training.
The images/val and respectively labels/val folders contain the data that is only used for validation, following the same naming conditions as the files in the train folders.

### Image Labeling
Images are labeled using [CVAT](https://www.cvat.ai/). There are two classes ``left_stick`` and ``right_stick``. For now, each stick in the image is labeled with a bounding box and two keypoints (for the stick tip and bottom). 
...
Simply execute ``data_preparation/data_preparation_script.py``. Change the path to the correct locations of your annotation file and specify an output folder before running the script.
...
```
### Run Training
To run a training, the first step is to adapt the ``config.yaml`` file. Change the first variable ``path`` to the path of your data folder. The other parameters should remain unchanged.
To run a training, execute the script ``train.py``.
Make sure that the variable ``data_path`` points to the config.yaml file. 
Results and weights of the training runs are saved in ``runs/pose`` folder.
## Performance and Results (for assignment 2)
### Data collected so far
I recorded videos of myself playing some common stick technique exercises and took the first ~10 seconds of the first video. From that, I exported every 5th frame and integrated it into the training data set. 
In total, I manually created data annotations for 68 video frames. By applying the flipping annotations script, I now have 136 labeled images in my training data set. 
### Performance metric
For measuring the performance I chose the mean average precision (mAP), which is calculated based on the Intersection over Unit (IoU) metric, which measures the overlapping of bounding boxes.
The mAP metric calculates the mean of the average precision values across all classes and/or IoU thresholds.
mAP50 considers thresholds of 0.5, covering the "easy" detections.\
mAP50-95 measures the precision for IoU thresholds between 0.5 to 0.95 and shows the model's performance concerning different detection level difficulties. 
### Performance measurements
| Run No. | Flipped Images | mAP50 | mAP50-95   | Epochs |
|-----|----------------|-------|------|--------|
| 4   | No             | 0.737  | 0.278 | 5  |
| 7   | No             | 0.911  |  0.509 |5     |
| 9   | Yes            |  0.976|  0.953    | 20     |

![results](runs/pose/train9/val_batch0_pred.jpg)

(Visual results for run no. 9)


## Outlook
My plan until the next assignment deadline is to create more training data with more variations in the images and play around with hyperparameters and the data augmentations provided by the YOLO framework.
Then the plan is to create a simple web page where one can upload a video of oneself drumming, and the website provides a video with tracked stick tips and optionally provides an average stick height metric for the left and right hand, as a use case on how this AI can help drummers in analyzing their drumming performance.
